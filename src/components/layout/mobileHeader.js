    ...
    <header className="py-4 scroll_hide  w-full flex lg:hidden  bg-transparent px-4 lg:px-12 ">
      <>
            ...
            <nav className="flex items-center gap-4 lg:hidden ">
              <section className="flex items-center gap-3">
                <div
                  role="button"
                  onClick={
                    user?.name ? () => router.push("/favorites") : () => router.push("/login")
                  }
                ...
                >
                  {user?.name ? (
                    <p className="uppercase">{`${user?.name?.charAt(0)}${user?.name?.charAt(
                      1
                    )}`}</p>
                  ) : (
                    <LuUser />
        ...
        <div
          ...
          } w-full bg-black transition-all   py-5 pt-4 px-7 absolute top-[110px] z-50 left-0  min-h-[10vh] overflow-hidden  `}
        >
          <div>
            <ul>
